...
| --- | --- | --- | --- |
| B1 | Complete | `4e34b7d` | A new `core/budgets.py`: `BudgetLedger` keeps counters in the memory store the application already chose, beside run state, keyed by scope, identity and window (`application:acme:2026-09-20`, `request:run_7:total`) over six meters. Every change is read-modify-write through the store's versioned compare-and-swap, retried on conflict, so six workers racing for the last dollar of a $3 budget spend exactly three and the other three are refused — proved on all four backends (in-memory, SQL, Redis, MongoDB), each of which gained `get_budget_state`/`save_budget_state` (a new SQL table, a Redis hash with a Lua compare-and-swap, a Mongo collection matched on version). Spending whose real cost is known only afterwards is reserved first and committed at its true cost; a process that dies in between leaves a standing reservation (a temporary over-count, never a lost spend), and the reservations of a run whose process died are released by run id when its lease expires. A refused charge spends nothing and reports the meter, limit, use and shortfall. A store without the two methods turns budgets off at the first call instead of failing the run. 18 new tests. |
| B2 | Complete | `c9523d4` | Budgets live in the policy envelope: `BudgetLimit` (meter, limit, window, `warn_at`, `on_exhausted`) and `PolicyBudgets` (per scope, with an `application_id` that an application-scope budget must name, since a request, session, and agent are identified by the run itself). Nothing is budgeted by default. A budget that cannot be read — an unknown meter or window, a limit of zero or less, a `warn_at` outside (0, 1], an unknown action, the same meter twice — is refused where it is written rather than guessed at. Budgets are part of what the policy hashes, so they cannot be widened at runtime without changing the policy's identity, and the run record's policy snapshot lists them. `governance_config["budgets"]` is the convenience for applications with no policy file; setting budgets in both places is refused. The old `policy.budget` keeps working and is expressed in the new shape as the agent's own limits, except a zero limit, which stays on the old field because that is how a subagent policy is narrowed today. 17 new tests. |
| B3 | Complete | `03752d2` | Budgets are enforced where the money is spent. A model call charges `model_calls`, then holds `model_cost_usd` at the most it could cost — the input counted from the messages just assembled, the output capped by `max_tokens`, both priced through the same LiteLLM table that already fills in `estimated_cost_usd` — and replaces the hold with the real figure when the answer lands (`estimate_model_call`, `RunBudgets.reserve/commit`). A call that fails releases its hold: nothing was spent. A model with no published price is not refused; its tokens and calls govern the run and it is recorded as `budget_cost_incomplete`. Tool calls are charged as they are authorized, sandbox seconds before each command and again at close (so a long session cannot overrun unnoticed), and delegations where they are authorized. Every level covering a run is charged — request, session, agent, application — so an application budget is shared by every run, proved against the store. Only budgeted meters are counted: nothing budgeted means nothing read or written. Crossing `warn_at` records `budget_warning` once; running out records `budget_exhausted` with scope, meter, limit and shortfall, and ends the run with `termination_reason="budget_exhausted"` rather than a provider error. A worst-case hold can refuse a call whose actual cost would have fit: that is the price of never overspending, and a test says so. Until B4, a budget set to `pause` stops the run like `terminate`. 12 new tests. |
| B4 | Complete | `6d1ffed` | A budget that runs out waits for a person instead of throwing the work away. The run stops where it is with `status="awaiting_budget"` and a `budget_request` saying which scope and meter ran out, what the limit is, what it has used and how much more it needs; the request is kept on the run's durable record, so the wait survives the process. `grant_budget(run_id, amount=..., approver=...)` adds to that one counter — a recorded exception with who granted it, when and why (`BudgetLedger.grant`, `granted`, `grant_history`), never a change to the policy, whose hash and limits stay exactly as written — and `resume(run_id)` carries the run on from its checkpoint. `deny_budget` ends the run on the next resume, and the refusal is remembered: the same budget terminates rather than asking the same person twice. Unattended jobs keep `on_exhausted: "terminate"`. Over HTTP: `POST /runs/{run_id}/budget` with `{decision: grant|deny, approver, amount?, note?}`, the waiting run's `budget_request` on `/run/sync` and its `budget_requests` on `GET /runs/{run_id}`. The decision is recorded in the resumed run's own trace (`budget_granted`/`budget_denied`), as an approval's is. 10 new tests, two of them over HTTP. |
