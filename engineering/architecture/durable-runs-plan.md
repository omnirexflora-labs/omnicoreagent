...
Status: complete (D1 to D6, 2026-09-19). Decisions confirmed by the maintainer (all four as recommended; run state in the chosen memory store, per the maintainer's revision).
...
| D5 | Complete | `13bd418` | Sandbox continuity. A paused, interrupted, or crashed run keeps no container: the run's sandbox scope closes when its call ends (tested: the container count is back to where it was while the run waits). The record notes `sandbox_used` when the run opens a sandbox session. On resume, after any pending calls' results (never between a tool call and its result), the model gets a runtime message (`sandbox_reset`, recorded in the trace like other runtime messages) that the sandbox was reset and the workspace files are intact. Tested against real Docker: a file written by a command before the pause comes back through the workspace bridge after resume, a file in `/tmp` does not, and a run that never opened a sandbox gets no notice. Docs on the Durable Runs page. Keeping or snapshotting a container across a pause stays later, per provider (Docker's snapshot does not capture the in-memory working directory; see E5b.2). 2 new tests. Full suite with live Redis and MongoDB 1,621 passed, 2 skipped; ruff clean; no containers left. |
| D6 | Complete | (this commit) | One story, background runs, and the end-to-end proof. `agent.get_run_trajectory(run_id)` returns every trace segment of a durable run (pause, resume, recovery, new attempt) in order, with totals summed across segments; a call that waited for approval and ran on resume is counted once, with its latest outcome (the first version counted it twice; caught by its test). Background: a new `awaiting_approval` run status (active, so it holds the task's slot; not claimable; no lease). The worker parks a run whose agent paused instead of completing it (before, it recorded any non-success agent status as a failed attempt) and emits `background_run_awaiting_approval`; `manager.resume_run(run_id)` queues it again after the approvals are decided (governed as a start when governance is on), and the next attempt continues the same durable run; cancelling a waiting run works; `run_until_terminal` and `wait_for_run` settle on a waiting run instead of polling until timeout. OmniServe: `POST /background/runs/{run_id}/resume`. End-to-end test: one governed run pauses for approval, gets one queued and one guardrail-blocked steering message while it waits, is approved and resumed, loses its process during the approved call, is recovered by a second agent on the same store after the lease, and finishes; nothing unapproved ran, nothing ran twice, the interrupted send is reported as unknown, the allowed steering message reached the model and the blocked one never did, and the run reads as one story. The threat model's open item on approvals is closed, with cited tests. Docs: background runs and the run trajectory on the Durable Runs page, and the route table. 5 new tests. Full suite with live Redis and MongoDB 1,625 passed, 2 skipped; ruff clean; no containers left. The durable runs plan is complete. |
