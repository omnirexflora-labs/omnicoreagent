...
| A6 | Complete | `e9bf824` | Subagents and background, measured. A delegation to a static sub-agent costs 116.7 ms CPU (the lead's two model calls plus the whole child run — about two requests' worth), builds nothing per request (no agent, recorder or memory store is constructed for a delegation; the worker is reused), and leaves no task or thread alive after the run. A background run of the same request cost 105.5 ms against 36.9 ms run directly, and made **36 task-store calls per run** — 10 re-reads of the task and 10 of the run, because every event written to the workspace fetched the task again to check a workspace policy that does not change during a run, and fetched the run again to find an object the caller already held; on a Redis or SQL task store each is a round trip. The event log now remembers, per run, the run object it was handed and the task's workspace policy read once, and forgets both when the run ends: 36 calls became 25 (task 10→3, run 10→6, three of which are the waiter's 50 ms polls), and the run's overhead beyond its request went from ~69 ms to ~34 ms. 1 guard. |
| A7 | Complete | `578d9f0` | Streaming, measured: 51 µs of CPU per delta, and with a 2,000-delta answer privacy redaction was 27 of 28.6 s under the profiler. Every delta's whole envelope — type, phase, identifiers, agent name — was walked and each string run through the four PII patterns, about five times per one-character delta; `actor_run_id` was not on the identifier list, so a phone-shaped run id could have been rewritten into a marker. Only the delta itself can carry personal data and it is redacted, once, before the envelope is put around it; `type`, `phase` and `actor_run_id` join the keys that are identifiers by construction. 28 µs per delta; telemetry records 11 events for a streamed request however many deltas it has, as before. 3 guards, including a delta with an email in it still coming out redacted. |
| A8 | Complete | `d6f0b52` | Leaks and lifetimes, measured over 400 requests on one agent: tasks alive, threads and open file descriptors stay flat (2, 3, 9) — no handle leaks; RSS grows about 160 KB per request and live objects about 560. By type, the growth is the in-memory telemetry mirror (each finished trace's 54 events, 14 spans, captures and actors) — retention by design, bounded at `memory_max_traces` (1,000), so a long-lived process plateaus at roughly 150 MB for the durable store's mirror of its own file — plus two per-request records that never went away: the run's record (history, by design) and **the request's own budget counter**, `request:<run_id>:total`, one key per request, never expired, on every durable store. Now, when a run ends (completed, failed or cancelled, not paused: a top-up lands on the counter), what it spent per scope is written on its record and its own counter is removed; session, agent and application counters are shared and stay. After 100 finished runs the store holds one budget key (the application's) where it held 101. `delete_budget_state` joins the store contract (in memory, SQL, Redis, MongoDB; a store without it leaves the counter). The in-process RSS curve is unchanged by this, as it should be: the leak it fixes is in the store, not the process. The telemetry mirror's size is a decision put to the maintainer. 2 guards. |
| A10 | Complete | `195d602` | Found by the final probe on a quiet machine, where a budgeted request cost twice a governed one: `copy.deepcopy` was the top self-time item, 54 top-level deep copies per request. By caller, 20 of them were the tool catalog: every step asked for the tool definitions and each definition deep-copied its schema again — for the model provider and the run record, which only read it — and the run-configuration record asked once more. The catalog already makes a private copy of each schema when it is built; a definition is now built around that copy and handed out to be read, so asking for the definitions again copies nothing, while the outer shape of what is handed out is still the caller's own to change. 54 deep copies per request became 39, and the 39 that remain are the in-memory store's own isolation on every read and write of run and budget state — a development store's cost, absent on SQL, Redis and MongoDB, which serialize instead. CPU could not be isolated on this machine at the time — the desktop was using a third of it — so the count is the measurement; the quiet-machine figures before this unit stand (no-op 18.8 ms, tool call 48.9, governed 47.4, budgeted 100.0), and the budgeted case is the one this unit lightens. 2 guards. |
| A11 | Complete | `5e5879e` | Decided with the maintainer ("yes add that"): the run record was rewritten for every message stored to history — 4 of a tool call's 11 saves and 2 of a no-op's 6 — beyond the durable-runs contract of step boundaries and write-ahead around tool calls. An assistant or user message is always followed by one of those saves before anything can go wrong (the write-ahead before a tool, the next step, the finish), so it now rides on it. A tool's *result* message is still written the moment it exists: the record keeps a completed call's state, not its result, and `_pending_calls` rebuilds a resumed run's conversation from `context.messages`, so without that save a crash between a call completing and the next step would leave a completed call the resumed run could not give to the model. That is why it is 11 → 8 rather than the 11 → 7 offered, and a guard crashes a run in exactly that window and finds the result on the record. 6 → 4 for a no-op. Every existing recovery, suspend, steering and end-to-end durable-run test passes unchanged. 3 guards. |
| A9 | Complete | `5e5879e` | `engineering/validation/performance-audit.md`: the method, one request before and after (140 → 18.8 ms CPU for a no-op on the audit machine), what one request does before and after, the eleven findings in the order they were found, the two things found not to be problems, the three decisions taken with the maintainer, what was left as it is on purpose, and how to take the numbers again. The audit plan is complete. |
