...
| S2b | done | | MongoDB a document at a time, as Redis is a key at a time: one write 145.5 ms -> 3.6 ms with 2000 runs held, and flat. Its store-wide lock is gone too, so no durable store has P2's failure mode. Contract suite, write-scope and two-manager races run on all four backends. Open: no backend prunes run history |
