...
| R5 | Done | (this branch) | The steward reports its own reward: `apps/steward/outcomes.py`, hourly from cron on the server. Recorded for the three pull requests it has opened so far — #250 closed unmerged (0.0), #251 and #252 merged (1.0). A run earns only the pull request its own `create_pull_request` call returned. |
