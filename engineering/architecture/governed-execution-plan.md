...
### E3. `execute` tool and skill scripts
- A built-in `execute` tool, present only when a sandbox backend is configured.
- `run_skill_script` runs in the sandbox, asynchronously; without a sandbox it
  follows the governance decision (a policy may still allow host execution
  explicitly, recorded as such).
- Both routed through governance (`process.exec`) and the governed tool runner.
