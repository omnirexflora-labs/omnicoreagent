...
| E5b.4 | Complete | `a53f46f` | Every run's `run_configuration` header lists `security_warnings` (a code and a message), and each is logged once when the agent initializes: `ungoverned_host_scripts` (skill scripts can run on the host with governance off: no policy and no sandbox), `sandbox_unused_without_governance` (a sandbox is configured but governance is off, so nothing runs in it; execution is only offered through governance, so this configuration silently does nothing), and `host_scripts_not_contained` (governance is on but there is no sandbox, so skill scripts are governed but not contained). The observability docs describe them. 4 new tests, including that a plain agent has none. Found during E5b.4: one full-suite run failed 12 Docker tests, all on a check that no sandbox container existed; the refusals themselves worked. One container from the unprivileged-user test was left running. It did not reproduce (the test alone 5 of 5 clean, with its neighbouring test files 3 of 3, then a clean full run), and the run started right after an interrupted session whose suite run may still have been going. The container was removed by ID. Those Docker checks now compare containers before and after instead of requiring none, so another run on the machine cannot fail them. Full suite 1,545 passed, 14 skipped; ruff clean; no containers left. |
| E5b.5 | Complete | `e6b77b6` | New `engineering/architecture/threat-model.md`: what is protected, who is assumed hostile (model output, code it runs, MCP servers), the trust boundaries, each of the ten incident failure classes with its controls, the test that proves each control, and what is still open; and a summary of what is not protected (application tools, host skill scripts, anything the policy allows, governance off, the host kernel without gVisor). A docs test fails if a cited test no longer exists (checked by adding a bogus citation). New user page `docs/core-concepts/security-model.mdx` (in the navigation): the two layers, the recommended setup, what the Docker sandbox enforces, a trust-boundary table, the built-in protections, and what is not protected. Found while writing the threat model: host skill scripts inherited the agent's whole environment, including `LLM_API_KEY` (failure class 3). Maintainer decision (2026-09-19): minimal environment plus opt-in. Host scripts now receive only `PATH`, `HOME`, the locale, `TMPDIR`, and `TERM`, plus the variables the application names in the new `skill_script_env` agent setting; the skills and configuration docs say so. A second skill-tools wrapper was missed at first and caught by the agent-level test. 6 new tests. Full suite 1,551 passed, 14 skipped; ruff clean; no containers left. E5b is complete. |
| E6a | Complete | `3375035` | Code mode: a `run_code` tool that runs a Python program in Monty (`pydantic-monty==0.0.23`, the new optional extra `omnicoreagent[codemode]`, pinned exactly since it is alpha). Monty's API was checked against the installed package, not from memory: it runs programs in its own worker subprocesses, a program's tool call arrives as a snapshot answered with a value or an exception, OS calls can be refused (the program sees `PermissionError`), and limits surface as errors. The program gets the chosen tools as functions (`code_mode.tools`, default every eligible tool; names must be identifiers; subagent delegation and tool discovery are never exposed), no filesystem, network, or environment access, and time, memory, tool-call, and output limits. Every call from a program goes through the same governed path as a direct call: `one()`'s handler selection, write-ahead run record, and governed runner are now one `dispatch()` used by both, so policy, records, and telemetry cannot differ between them. Inner calls take the ID `<run_code id>.<n>` with a `parent_tool_call_id`, are listed under their `run_code` call in the trajectory (`code_calls`), and a denied call raises an error the program can catch. `run_code` is governed as its own capability (`code.run`, surface `code`, medium risk), allowed by the dev profiles (`allow_code_mode`) and refused by strict-production. Config `agent_config["code_mode"]`, validated. 6 new tests (a program's result, output, and two governed calls; a denied call refused and nested in the trajectory with the run record's parent links; no filesystem and only the chosen tools; time and tool-call limits; not offered when disabled; its capability). Found: `uv sync` with only the new extras pruned the OpenTelemetry extra, which skipped an OTLP test; the environment now installs docker, codemode, and otel. Full suite with live Redis and MongoDB 1,632 passed, 2 skipped (S3 and R2); ruff clean. |
| E6b | Complete | `1bf7a7b` | Approval inside a program. When a tool call a program made needs a person's decision, the paused program is dumped to bytes, signed (HMAC with `code_mode.snapshot_key`, `OMNICOREAGENT_CODE_SNAPSHOT_KEY`, or a per-process key), and stored on the run record with the paused call's ID and number; the run suspends like any other, and the approvals returned name the call inside the program. On resume the program is restored and continues from that call, so nothing it already did runs again; the decision reaches it as the call's result, or as a catchable error carrying the approver's note when denied. A stored program that does not verify is refused and never runs, and one larger than `max_snapshot_bytes` is told so instead of pausing. A `run_code` call counts as waiting while any call it made is. Found during E6b: on an async session `load_snapshot` returns an awaitable, which the first version did not await (the program silently failed to resume); the resumed program now runs. 4 new tests (pause then continue where it stopped; a denial raised inside the resumed program; a tampered stored program refused; a second agent with the same store and key resumes it, as after a restart). Full suite with live Redis and MongoDB 1,636 passed, 2 skipped; ruff clean. E6 is complete. |
| E7a | Complete | (this commit) | Modal sandbox backend (`sandbox/modal_sandbox.py`, provider `modal`, optional extra `omnicoreagent[modal]`). Each session is one Modal Sandbox: the manifest's image, working directory, environment, CPU, and memory, no network by default, a session timeout, and commands through `exec` with a per-command timeout; files use Modal's filesystem API and must stay inside the working directory; `terminate` ends the session. Modal can enforce a network host allowlist (`outbound_domain_allowlist`), so unlike Docker an allowlist is honoured instead of refused; changing the policy after a session starts is refused, because Modal fixes it at creation. The whole SDK surface was checked against the installed package rather than from memory (every method has an `.aio` counterpart). 6 new tests: a stand-in SDK checks exactly what the adapter asks Modal for (image, app, workdir, environment, CPU, memory, blocked network; the allowlist; commands, files, termination; a path outside the working directory refused; the provider registered), and a live test runs against the machine's configured Modal account, skipped with its reason where there is none. Found by the live test: the file API first used was Modal's legacy sandbox filesystem, which Modal now refuses; the adapter (and the stand-in) use the current `filesystem.write_bytes` / `read_bytes` / `make_directory`. The live run created a real sandbox, ran a command, moved a file, and confirmed the sandbox has no network. Full suite with live Redis, MongoDB, and Modal 1,642 passed, 2 skipped; ruff clean. |
