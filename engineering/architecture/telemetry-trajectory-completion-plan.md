...
- Rejections name their reason (`invalid_arguments`, `unknown_tool`,
  `arguments_rejected`); governance events carry their identity (request,
  decision, effect, capability, `tool_call_id`) in metadata so it survives
  restricted capture. Found during B4: the `subagent.spawn` authority request
  recorded the full delegated task text; that field is now redacted (its
  length is still recorded). Tool deadlines run inline (`stop_after`) because
  an extra task per tool let a batch cancel discard results that had already
  been produced.
...
- Runtime-injected messages (stuck and empty-response nudges) are events.
  Found during B5: the current-datetime prefix added to the user's message is
  also runtime text; it is recorded the same way, so the recorded user message
  and what the model saw can be reconciled.
...
- The summary is a pure function of the stored trace (`summarize_trace`), so
  the reader (B7) reuses it; failed, cancelled, and timed-out runs record
  their totals too. Found during B6: under a server that cancels repeatedly
  (anyio), both the serve trace and the agent trace were left `running`
  because cleanup was cancelled again; cleanup now completes within a bound
  (`complete_despite_cancellation`, shared with the background supervisor),
  and its context changes are carried back to the caller.
...
- OmniServe exposes the same structure at
  `GET /telemetry/runs/{run_id}/trajectory` (and by trace ID).
- Every event appears exactly once; unplaced events are listed in
  `other_events` where they occurred. The builder is a pure function of stored
  traces (a reloaded file yields the identical trajectory).
- Found during B7 by reading a live run: the PII phone pattern redacted ISO
  dates and timestamps (`2026-09-18 12` became `[REDACTED_PHONE]`) wherever
  they appeared in recorded text; dates are now kept and phone numbers are
  still redacted.
...
- The schema ships inside the package (validation at runtime needs it) and is
  kept identical to the published copy by a test; it now also requires a
  `reason` for unavailable capture states, as the specification states.
  References in `facts` / `final_output_references` must resolve.
...
Result: [trajectory acceptance report](../validation/trajectory-acceptance.md).
Found during C1: a store shared across event loops dropped contended writes
(one lock per loop now), and PII redaction corrupted about 1 % of provider
response IDs by matching digit groups inside UUIDs (UUID matches now kept).
...
| B3b | Complete | `1d21d5b` | 5 new tests: provider cost and standard usage fields on span and event; price-table cost when the provider reports none; unknown cost stays unknown; fields survive JSONL reload and reach the OTel mapping (`gen_ai.usage.*`); the cached-input rate is used. Live LiteLLM: streamed (price table) and non-streamed (provider) costs match exactly for identical usage ($0.00052305). Full suite 1,269 passed, 14 skipped; acceptance checks passed; ruff clean. |
| B4 | Complete | `c0c5587` | 10 new tests on a real `OmniCoreAgent` run: malformed arguments keep the raw text and reason; unknown tools are rejected with their own reason; a tool timeout is `timeout` with an error event while a cancelled run's tool stays `cancelled`; tool records link to the model call, response, batch, and requested/resolved events; governance records redacted arguments and links decisions to the call, also under restricted capture; subagent calls report the `subagent` provider; delegation parameters are redacted under governance; `stop_after` marks timeouts only. Full suite 1,279 passed, 14 skipped; acceptance checks passed; ruff clean. Observed once, not reproduced in two reruns: a SQLAlchemy `TypeError` in the setup of the `[sql]` native-runtime fixture (no telemetry code involved). |
| B5 | Complete | `8b7429a` | 8 new tests on a real `OmniCoreAgent` run: observations link to their tool result event and span (and to the request for rejected calls); the next context and model call list the observations they deliver, new only once, including parallel calls in order; datetime prefix, empty-response retry, and loop recovery are `runtime_message` events whose digests match the next context; model calls record `agent_turn` or `context_summary`. Full suite 1,287 passed, 14 skipped; acceptance checks passed; ruff clean. |
| B6 | Complete | `3cfd261` | 9 new tests: totals for a successful run (steps, model calls, tokens, cost, tool outcomes incl. rejected and error), timeouts and governance denials, workspace changes, a model-failure result, a raising run, cleanup context propagation, an agent error served as `failed`, and a cancelled served request leaving neither trace running. 2 existing tests updated for the new `run_summary` in the root output. Live LiteLLM: summary totals equal the per-call sums (3,056 tokens, $0.0017208) and the final answer links its model response. Full suite 1,295 passed, 14 skipped; acceptance checks passed; ruff clean. |
| B7 | Complete | `9af3ff5` | 9 new tests: a full-capture run read from request to final answer (parallel success/error/rejected calls, observations equal to the tool messages the next call sent, final answer linked to its model response); a child run nested under its delegating call; every event accounted for (the test fails when the builder drops an event type); a reloaded file yields the identical trajectory; default capture keeps structure and states what is missing; lookup by run ID; the OmniServe routes; dates kept and phone numbers redacted. Live LiteLLM run read end to end through the reader. Full suite 1,304 passed, 14 skipped; acceptance checks passed; ruff clean. |
| B8 | Complete | `2056199` | 11 new tests: packaged and published schemas identical; a current runtime export validates and malformed `run_summary` / `purpose` / token values are rejected; schema-valid extensions (extra fields, unknown span kind and actor type) import cleanly; invalid documents raise only `EvidenceValidationError`; a claimed `complete` with capture gaps is recomputed to `partial`; unresolved references are rejected; the generic importer records inferred/missing event type, actor, span, kind, and cost, downgrades a `complete` claim, and never modifies its input. The three committed acceptance fixtures validate against the stricter schema and keep their evidence status. Full suite 1,315 passed, 14 skipped; acceptance checks passed; ruff clean. Phase B complete. |
| C1 | Complete | `77cfe15` | `trajectory_acceptance.py`: all 10 checklist items pass directly (full and default capture) and through OmniServe; the sanitized fixture passes the standard-library check; the suite runs both (`tests/test_trajectory_acceptance.py`). Live LiteLLM (`gpt-5.4-mini`): 3,060 tokens (2,048 cached), $0.0010326, 25 of 25 events accounted for, evidence complete, key absent. 5 new tests (cross-loop store, UUID identifiers, hyphen-joined phone). Full suite 1,320 passed, 14 skipped; all acceptance checks passed; ruff clean. |
| C2 | Complete | (this commit) | Coverage map rewritten against the actual emitters and tests (the old map's observation claims corrected); evidence spec gains the concrete link fields and a "Reading a run" section; telemetry spec, migration plan, and its superseded storage and retention decisions updated; observability guide checked and already current. Found during C2: under machine load the served acceptance run's delegation hit the scenario's 1 s tool limit, which the script set by mutating the agent after construction, so the run header recorded 180 while 1 was enforced. The scenario now configures `tool_call_timeout: 3` through `agent_config`, asserts the header records it, and the fixture was regenerated; 5 of 5 runs pass at load average 7 to 9. Full suite 1,320 passed, 14 skipped; all acceptance checks passed; ruff clean. Plan complete. |
