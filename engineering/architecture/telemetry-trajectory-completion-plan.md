...
| C1 | Complete | (this commit) | `trajectory_acceptance.py`: all 10 checklist items pass directly (full and default capture) and through OmniServe; the sanitized fixture passes the standard-library check; the suite runs both (`tests/test_trajectory_acceptance.py`). Live LiteLLM (`gpt-5.4-mini`): 3,060 tokens (2,048 cached), $0.0010326, 25 of 25 events accounted for, evidence complete, key absent. 5 new tests (cross-loop store, UUID identifiers, hyphen-joined phone). Full suite 1,320 passed, 14 skipped; all acceptance checks passed; ruff clean. |
