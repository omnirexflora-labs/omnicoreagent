...
| Step | What happens |
| --- | --- |
| 1 | One parallel batch: `lookup` succeeds, `explode` raises, `lookup` with the malformed arguments `{broken` is rejected, `slow` exceeds the 10 s tool limit (`tool_call_timeout`, recorded in the run header), and `big_report` returns a result large enough to be offloaded to a workspace artifact. |
