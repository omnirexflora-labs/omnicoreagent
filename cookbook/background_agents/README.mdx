# Background Agents: Scheduled Automation Examples

> Background Agents let you run scheduled agent tasks without a human prompt each time. This cookbook section is currently an experimental scheduled-automation path and will get a deeper production refresh after the core runtime and OmniServe work.

## Why Background Agents?

OmniCoreAgent's background system is designed around scheduled task execution:
*   **Timeouts and retries**: Configure how long a task may run and how failures are retried.
...
*   **Events**: Background runs emit structured events you can inspect.
*   **Lifecycle control**: Pause, resume, update, or stop agents while the manager is running.
...
Install the background extra first:
...
Set `queue_size` on the top-level agent config to control the worker queue depth.
