...
| 12 | [agent_with_sub_agents.py](./agent_with_sub_agents.py) | **🆕 Sub-Agents**: Build multi-agent systems |
| 13 | [agent_configuration.py](./agent_configuration.py) | **Advanced Config**: All settings in one place |
...
| Track usage for cost estimation | [agent_with_metrics.py](./agent_with_metrics.py) |
...
Run `python cookbook/getting_started/agent_with_streaming.py` to consume live text
and a terminal outcome. Intermediate text is not the final answer; check the
terminal status. Closing the iterator cancels the run.
