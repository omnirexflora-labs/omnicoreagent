...
explicit subagents when you want a fixed team.
...
- **Workspace Continuity**: Dynamic subagents write findings to workspace memory, so outputs survive context compression and tool offloading.
...
agent = OmniCoreAgent(
    ...
    system_instruction="You coordinate deep research and synthesize findings.",
...
When `enable_subagents` is true, OmniCoreAgent also enables workspace memory.
Dynamic workers are expected to save findings with `memory_create_update`, and
the lead agent reads those paths with `memory_view` before synthesizing.
...
[
  {
    "name": "market",
    "role": "Market analyst",
    "task": "Research market size, growth, competitors, and risks.",
    "output_path": "/memories/market_entry/subagent_market/findings.md"
  },
  {
    "name": "technical",
    "role": "Technical analyst",
    "task": "Assess architecture, integration cost, and implementation risks.",
    "output_path": "/memories/market_entry/subagent_technical/findings.md"
  }
]
```
