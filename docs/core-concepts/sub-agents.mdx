...
When `enable_subagents` is true, OmniCoreAgent also forces workspace files on
even if it is omitted. Keep it explicit in app configs because dynamic workers
are expected to save output with `workspace_file_write`, and the lead agent
reads those paths with `workspace_file_view` before synthesizing.
...
workspace files, context management, and tool offloading. They do not inherit
