...
| `allowed-tools` | no | Space-separated tool names. Parsed and kept with the skill; it neither restricts nor pre-approves anything. The specification calls it pre-approved tools, but a skill cannot approve its own actions: the [policy](/docs/core-concepts/security-model) decides every call, so write allow rules there ([rule syntax](/docs/reference/policy)) |
...
header (`trajectory["harness"]["security_warnings"]`) and logs it once at
start-up. Only switch on skills you trust, or turn
