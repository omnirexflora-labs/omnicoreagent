---
title: Agent Skills
description: 'Self-contained capability packages for specialized agent tasks'
icon: 'graduation-cap'
---

# Agent Skills System

OmniCoreAgent supports the **Agent Skills** specification — self-contained capability packages that provide specialized knowledge, executable scripts, and comprehensive documentation for agents.

## What is an Agent Skill?

An agent skill is a directory following a specific structure that packages everything an agent needs to perform specialized tasks. Skills allow you to build portable, reusable capabilities that can be shared across agents and projects.

### Directory Structure

```text
.agents/skills/my-skill-name/
├── SKILL.md        # The "Activation" document (instructions + metadata)
├── scripts/        # Multi-language executable scripts (Python, JS, etc.)
├── references/     # Deep-dive documentation and research
└── assets/         # Templates, examples, and other resources
```

---

## Configuration

Enable skill discovery in the agent configuration:

```python
from omnicoreagent import OmniCoreAgent

agent_config = {
    "enable_agent_skills": True,  # Enable discovery and tools for skills
}

agent = OmniCoreAgent(
    name="assistant",
    system_instruction="You are a helpful assistant.",
    model_config={"provider": "openai", "model": "gpt-5.6-terra"},
    agent_config=agent_config,
)
```

---

## How It Works

1. **Discovery**: The agent automatically scans the `.agents/skills/` directory at startup.
2. **Activation**: The agent is instructed to read the `SKILL.md` file first to understand the skill's capabilities and how to use them.
3. **Execution**: The agent uses specialized tools to interact with skill files and run scripts.

### Skill-Specific Tools

| Tool | Description |
|------|-------------|
| `read_skill_file` | Access any file within a skill (docs, references, etc.). |
| `run_skill_script` | Execute bundled scripts with automatic interpreter detection. |

---

## Polyglot Script Execution

The `run_skill_script` tool supports multiple languages, allowing you to bridge different ecosystems:

- **Python** (.py)
- **JavaScript / Node.js** (.js)
- **TypeScript** (.ts)
- **Ruby** (.rb)
- **Bash / Shell** (.sh)

The runtime automatically detects the correct interpreter and passes arguments from the agent to your script.

### Where scripts run and what they can see

When the agent has a sandbox (governance enabled with a sandbox that can run
commands), a script runs in that sandbox: the skill's files are copied in and
the script cannot see the host. Otherwise it runs on the host, as your
process's user.

On the host, a script does not inherit the agent's environment. It receives
only `PATH`, `HOME`, the locale (`LANG`, `LC_*`), `TMPDIR`, and `TERM`, so it
cannot read `LLM_API_KEY` or any other secret your process holds. Pass a
variable a skill needs by name:

```python
agent_config = {
    "enable_agent_skills": True,
    "skill_script_env": ["MY_SERVICE_URL"],
}
```

Running a script is governed as `skill.script.run`, a high-risk capability on
the `host` (or `sandbox`) surface, so a policy can allow, ask for, or deny it.

---

## Creating Your Own Skills

A skill is a folder under the skills directory (`.agents/skills/` in the
working directory, or `agent_config["skills_dir"]`), named after the skill:

```text
.agents/skills/citation-formatter/
├── SKILL.md
└── scripts/
    └── format_sources.py
```

`SKILL.md` starts with YAML frontmatter. `name` and `description` are
required, and `name` must equal the folder's name, or the skill is skipped
with a warning. The rest of the file is the instructions the agent reads.

```markdown
---
name: citation-formatter
description: Formats a list of sources as numbered citations.
---

# Citation formatter

Run `format_sources.py` with one argument per source, then paste its output
under a "Sources" heading.
```

The agent reads a skill's files with `read_skill_file` and runs a script
with `run_skill_script(skill_name, script_name, args=[...], timeout=30)`:
`script_name` is relative to the skill's `scripts/` folder
(`"format_sources.py"`), and `args` is a list of strings. Optional
frontmatter fields: `license`, `compatibility`, `metadata`, and
`allowed-tools` (space-separated). The format follows
[agentskills.io](https://agentskills.io/).

### Tips for Skill Authors

- **Clear Metadata**: Ensure your `SKILL.md` has a clear "Purpose" and "Usage" section.
- **Example Driven**: Include examples of successful tool calls or script executions.
- **Portable Scripts**: Avoid hardcoded paths in your scripts; use the environment and arguments provided by the agent.
