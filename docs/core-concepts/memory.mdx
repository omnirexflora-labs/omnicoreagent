...
| Backend | Use Case | Environment Variable | Install |
|---------|----------|---------------------|---------|
| `in_memory` | Development; gone when the process ends | — | — |
| `sql` | SQLite on one machine, PostgreSQL or MySQL when processes share it | `DATABASE_URL`, e.g. `sqlite:///./memory.db` or `postgresql://...` | `omnicoreagent[postgres]` (SQLAlchemy; also used for SQLite) |
| `redis` | Production persistence | `REDIS_URL` | `omnicoreagent[redis]` |
| `mongodb` | Document storage | `MONGODB_URI` | `omnicoreagent[mongodb]` |

A run's durable record lives in the memory store too, so pausing a run in one
process and resuming it in another needs a store both can reach: `sql`,
`redis` or `mongodb` ([durable runs](/docs/core-concepts/durable-runs)).

```python
import os
os.environ["DATABASE_URL"] = "sqlite:///./memory.db"
