...
|------------|--------------------|---------------------|
| **Native tool-call contract** | Provider-native function calls retain IDs and typed JSON arguments; text remains task content. Configured children use native delegate tools. | `src/omnicoreagent/core/model_protocol.py` |
| **Parallel batch tool execution** | Independent tools from one model step are resolved and executed together with timeout handling. | `src/omnicoreagent/core/agents/native_tools.py` |
...
| **Structured observations** | Tool outputs are normalized, guarded, formatted, and returned to the model as structured observations. | `src/omnicoreagent/core/tools/tool_result_offloader.py` |
...
| **Loop detection** | Complete native tool rounds are compared using provider/server identity, arguments and results; repeated rounds/cycles disable tools. | `src/omnicoreagent/core/agents/loop_detection.py` |
...
4. Call LiteLLM
5. Normalize the provider turn, retaining text, calls, finish reason and usage
...
The default agent includes the protections needed for reliable multi-step
execution. Capabilities that change the tool surface or require external
...
| Context management | On | Keeps active messages within the configured budget before each model call. |
| Tool offload | On | Keeps large tool payloads in workspace artifacts and sends a preview to the model. |
...
`OmniCoreAgent` is the public agent for both normal and deep runs. Enable
`agent_config={"enable_subagents": True}` to register the native `spawn_subagents`
tool and let the parent create focused workers with a shared workspace. Both
`run()` and `stream()` support this mode. There is no separate `DeepAgent` class.
Configured children can also be supplied through `sub_agents`.

The facade builds this capability through `core/runtime/builder.py` and
`core/runtime/harness_tools.py`; worker creation lives in `core/subagents.py`.
Background execution is a separate scheduling and persistence layer around the
same agent runtime, exposed through `BackgroundAgentManager`. It provides durable
job lifecycle and retry handling. Retired router/sequential/parallel workflow
classes are not required for native parallel tool calls or child batches.
