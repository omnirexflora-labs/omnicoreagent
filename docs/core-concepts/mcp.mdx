...
Hosted MCP servers work the same way. You.com runs a public MCP server for
live web search and URL content extraction — no local process needed:

```python
# Keyless profile: basic web search, no API key or OAuth required
{
    "name": "you_search",
    "transport_type": "streamable_http",
...
# Authenticated: full You.com toolset with a Bearer token
# (API keys: https://you.com/platform/api-keys)
{
    ...
    "headers": {
        "Authorization": "Bearer your-youcom-api-key"
    },
    "timeout": 60
}
```
