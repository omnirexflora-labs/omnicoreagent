...
graph TD
    ...
    Tools --> MCP["MCP tools"]
    Tools --> WorkspaceTools["Workspace command tools"]
    ...
    State --> Telemetry["Telemetry stream"]
...
- agent configuration
- memory routers and telemetry streams
...
| Workspace command tools | File operations for notes, scratchpads, task progress, and generated files. |
| Artifact tools | Read, tail, search, and list offloaded tool-result artifacts. |
...
|---------|----------------|
| Memory router | Conversation/session history, and every run's durable record (step, tool calls, approvals, budgets) and the budget ledger. |
| Telemetry stream | Typed run, tool, background, and service events for live streaming and replay. |
| Workspace storage | Files used by agents, subagents, tools, artifacts, scratchpads, and offloaded payloads. |
| Governance engine | The policy, its evaluator, the approval resolver, the sandbox runtime and manifest: every capability a run uses passes through it ([security model](/docs/core-concepts/security-model)). |
| Sandbox execution | One sandbox session per run, opened by the first command and closed with the run; the workspace bridged in before each command and back after ([execution](/docs/core-concepts/execution)). |
