...
The durable task stores use optional drivers; the SQL store needs SQLAlchemy,
which the `postgres` extra installs, for SQLite as well:

```bash
pip install "omnicoreagent[postgres]"   # task_store="sql": SQLite or PostgreSQL
...
Every method, run status and overlap policy is in the
[BackgroundAgentManager reference](/docs/reference/background).
...
An interval task first runs one interval after it is registered (or after its
`start_at`), not at once; call `run_now` for a run right away.
...
Under governance a task is bound to the policy it was created under. When the
policy changes, the task can still be paused or deleted, but not run: at its
next due time its schedule is paused, the task's status shows
`schedule_state.paused_reason`, and the log says why. Delete the task and
create it again under the new policy to schedule it.
...
The SQL store keeps a row per agent, task, schedule, run and attempt.
`task_store="sql"` reads `DATABASE_URL`, the same variable the SQL memory store
uses, and falls back to a local SQLite file.

A write costs what one row costs, not what the store holds: with two thousand
runs kept, one run write takes about 3 ms on SQLite and 7 ms on PostgreSQL,
against 49 ms for the older whole-state SQL store and 104 ms on Redis. Run
history is not pruned, so a long-lived deployment on Redis or MongoDB gets
slower; on SQL it does not.
...
```python
BackgroundAgentManager(task_store="sql")  # SQLite, or DATABASE_URL if set
BackgroundAgentManager(
    task_store={
        "backend": "sql",
        "url": "postgresql://user:pass@localhost:5432/omnicoreagent",
    }
)
