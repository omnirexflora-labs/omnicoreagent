...
| Sandbox | The `execute` tool and skill scripts | A container ([sandbox providers](/docs/how-to-guides/sandbox-providers)), or this machine with `local` |
...
| `process.exec` | Each command inside the sandbox | Allowed on the sandbox surface, never on the host (the `local` provider's surface) |
