...
run's session opens, a command inside it — and pauses the run the same way,
recorded against the `execute` call that needed it.
...
again with `resume_run` or `POST /background/runs/{run_id}/resume`.
