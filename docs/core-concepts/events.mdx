...
| `runtime_error` | Runtime failed: the error's type and message; with `capture: "full"`, its traceback too |
...
| `background_run_recovered` | Expired lease was recovered and requeued; `resumed: true` when the next attempt continues the run from its checkpoint instead of starting over. |
