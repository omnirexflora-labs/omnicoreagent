...
Without `amount`, a grant covers the shortfall of the one call that stopped:
the run can stop again at its next call. To give it headroom, grant more,
e.g. `grant_budget(run_id, approver="alice", amount=0.50)`. A grant adds to
what the budget allows; `budget_status(run_id)` shows each budget's `limit`,
what was `granted`, what is `spent`, and what `remaining`.
...
<CardGroup cols={2}>
  <Card title="Policy reference" icon="shield-halved" href="/docs/reference/policy">
