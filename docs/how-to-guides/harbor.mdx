...
| `completion_review` | `0` | Rounds of review before a final answer is accepted: the agent is asked for each requirement and the check that showed it, and keeps working where it has none. Its report is more thorough; on a hard task it did not raise the score and doubled the cost. |
