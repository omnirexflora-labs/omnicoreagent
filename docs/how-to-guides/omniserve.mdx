...
The file may also define its own routes — an application's pages, its own
endpoints — mounted beside the agent's API on the same origin, behind the same
middleware:
...
router = APIRouter()            # or routers = [...]

@router.get("/steward/", response_class=HTMLResponse)
async def page() -> str:
    return open("page.html").read()

public_paths = ["/steward/"]    # served without a token; "/steward/*" for a directory
```

Everything else the router serves still needs the bearer token when auth is
on. In code, the same are `OmniServe(agent, config, routers=[...],
public_paths=[...])`.
</Info>
...
Startup takes a few seconds. Wait for `GET /ready` to return `ready: true`
before sending work.
...
`omniserve quickstart --provider openai --model gpt-5.4-mini`.

**Examples:**

```bash
# OpenAI
omniserve quickstart --provider openai --model gpt-5.4

# Google Gemini
omniserve quickstart --provider gemini --model gemini-3.8-flash

# Anthropic Claude
omniserve quickstart --provider anthropic --model claude-sonnet-5
```
...
| `GET` | `/runs/{run_id}` | Yes* | A run's durable record: status, step, usage, tool call states, approvals (never its saved conversation) |
| `GET` | `/runs/{run_id}/trajectory` | Yes* | The whole run as one story: every trace segment in order, totals across them, each tool call once with its final outcome, the approvals |
| `POST` | `/runs/{run_id}/approvals/{approval_id}` | Yes* | Decide an approval a paused run waits for: `{"decision": "approve" \| "deny", "approver", "note"?, "arguments"?}` |
| `POST` | `/runs/{run_id}/resume` | Yes* | Continue a paused run once its approvals are decided |
| `POST` | `/runs/{run_id}/steer` | Yes* | Send a running, waiting, or interrupted run a message for its next step: `{"message", "sender"?}` (422 if the guardrail blocks it) |
| `POST` | `/runs/{run_id}/interrupt` | Yes* | Stop a running run at its next step boundary; resume continues it |
| `GET` | `/runs/{run_id}/budget` | Yes* | Every budget covering the run, with its limit, what is spent, and any pending request |
| `POST` | `/runs/{run_id}/budget` | Yes* | Grant or refuse the budget a waiting run ran out of: `{"decision": "grant" \| "deny", "approver", "amount"?, "note"?}` |
| `POST` | `/runs/{run_id}/outcome` | Yes* | Record what the run turned out to be worth: `{"source", "reward"?, "label"?, "detail"?}` |
| `GET` | `/sessions/{session_id}/history` | Yes* | A session's message history |
| `DELETE` | `/sessions/{session_id}` | Yes* | Clear a session's message history |
...
| `GET` | `/telemetry/traces/{trace_id}/family` | Yes* | Return all traces linked to one execution boundary |
...
| `GET` | `/telemetry/runs/{run_id}/family` | Yes* | Every trace linked to one run by parent trace links |
| `GET` | `/telemetry/runs/{run_id}/trajectory` | Yes* | The run's latest trace segment as a trajectory; `/runs/{run_id}/trajectory` is the whole run |
| `GET` | `/telemetry/traces/{trace_id}/trajectory` | Yes* | One trace as a trajectory |
| `GET` | `/telemetry/retention` | Yes* | The trace and payload retention policy and the latest cleanup results |
...
readiness. The response's `mcp_servers` gives each configured server's status
(`connected`, `disconnected`, `failed`, `not_connected`) and last error, so a
single failed server among several is visible.
...
Under governance, what an operator asks here is decided by the served agent's
policy too: creating, changing, pausing, resuming or deleting a task is
`background.task.<action>`, and starting or cancelling a run is
`background.run.<action>`. A policy that does not allow them refuses the
request with `403` and the reason (a `strict` policy refuses anything no rule
covers). Allow them for the operators you trust — see the
...
| `POST` | `/background/runs/{run_id}/resume` | Yes* | Queue a background run that was waiting for approval, once its approvals are decided |
...
The generator creates a Dockerfile for the current project directory and stores
only non-sensitive defaults in the image:
...
does not import or execute the agent file. S3/R2 workspace credentials are
passed at runtime with `-e`, never baked into the image.
...
The SSE `/run` endpoint emits `text_delta` while the provider response is still in
progress, in addition to lifecycle events and terminal `complete` results. Each
delta is intermediate and includes actor identity and a sequence number. Deltas
are delivered even when raw model-response trace recording is disabled. They are
live-only; the session event endpoint replays persisted lifecycle/final-answer
records, subject to recording configuration. A disconnect cancels the active run.
Check the final payload's `status` and `termination_reason` before treating a run
as successful. See [basic usage](/docs/how-to-guides/basic-usage) for the Python contract.
