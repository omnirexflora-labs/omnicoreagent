...
1. Local tools, MCP tools, workspace/artifact tools, skills, and other registered runtime tools form an isolated catalog for each run.
...
5. Matching native tool schemas become available on the next model request. A discovery call cannot unlock another call in its own batch.
...
- **Container-friendly** — no separate search service
