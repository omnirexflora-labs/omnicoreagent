...
`governance_config.sandbox_config` names the sandbox the agent's `execute`
tool runs commands in: `docker`, `e2b`, `modal`, `daytona`, `vercel`, `http`
(your own service), or `local` (the machine the agent runs on, not isolated),
plus any provider you register. `none` gives no sandbox (policy only), and
`local_test` runs registered handlers for tests. What each needs and enforces —
including which ones can limit a sandbox's network to named hosts — is on
[Sandbox providers](/docs/how-to-guides/sandbox-providers).
...
- **Model Selection**: Use smaller models (`gpt-5.4-mini`) while building and testing your agent logic to save costs.
