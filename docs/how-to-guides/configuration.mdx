...
agent = OmniCoreAgent(
    ...
    telemetry_config={
        ...
        "capture": "default", # default (privacy-first) or full
        "record_inputs": True,
        "record_outputs": True,
...
`capture: "default"` is privacy-first: model prompts and responses are not
recorded, and the trace marks them `not_recorded`. `capture: "full"` records the
complete trajectory, including model input and output, still redacted,
truncated, and offloaded under the same policy. Any `record_*` field you set
