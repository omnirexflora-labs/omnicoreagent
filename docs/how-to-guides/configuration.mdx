...
agent = OmniCoreAgent(
    ...,
    telemetry_config={
        "storage": "auto", # memory, jsonl, or auto
        "record_inputs": True,
        "record_outputs": True,
        "record_tool_results": True,
        "record_model_prompts": False,
        "record_model_responses": False,
        "retention_days": 7, # use None for unlimited local JSONL retention
...
each agent trace. With `storage: "auto"`, an explicitly configured local
workspace selects the built-in JSONL store; callers without a durable workspace
keep the in-memory default. Set `storage: "memory"` or `storage: "jsonl"` to
override that choice, and set `storage_path` for an explicit JSONL location.
