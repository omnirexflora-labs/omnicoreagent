...
each agent trace. The default store is in-memory for lightweight local use;
pass a `JsonlTelemetryStore` when traces must survive a process restart. An
exporter is an optional outbound copy of a completed built-in trace and is not
part of the runtime execution path.
